...
const getCanvasComponent = (mode, children) => {
  switch (mode) {
    case APPLICATION_MODE.AUDIO_SCOPE:
      // eslint-disable-next-line react/no-children-prop
      return <AudioScopeCanvas children={children}/>;
    default:
      // eslint-disable-next-line react/no-children-prop
      return <Visual3DCanvas mode={mode} children={children} />;
  }
};
...
const AudioPlayer = ({startMode, audioSource, children}) => {
  ...
  return (
    <Suspense fallback={<span>loading...</span>}>
      ...
      {getCanvasComponent(mode, children)}
    </Suspense>
  );
};
